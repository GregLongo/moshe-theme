...
function _moshe_file_types($mimes = array()) {
	$mimes['svg'] = 'image/svg+xml';
	return $mimes;
}
add_action('upload_mimes', '_moshe_file_types');

function _moshe_copyright_years($starting_year) {
...
function _moshe_prepare_acf_json($groupID, $actuallyPrepare = true) {
	if($actuallyPrepare) {
...
function _moshe_get_field_group_data($target = false, $excludeTarget = false, $prepareJSON = true) {
	$ids = array(
	...
	if($target) {
		...
		if(!$excludeTarget) {
			return _moshe_prepare_acf_json($ids[$target], $prepareJSON );
		}
	...
	return array_map(
		function($el) use ($prepareJSON) {
			return _moshe_prepare_acf_json($el, $prepareJSON);
		},
		array_values($ids)
	);
}

function _moshe_wrap_if($text, $condition, $prefix, $suffix) {
	if($condition) {
...
function _moshe_get_latest_blog_posts($num = 1) {
...
function _moshe_render_menu_recursively($menu_items) {
	$o = '<ul>';
	...
	foreach($menu_items as $item) {
		...
		if(!empty($item->children)) {
			$o .= _moshe_render_menu_recursively($item->children);
		}
		$o .= '</li>';
	}
 ...
 * @return [type]
 */
function _moshe_layout_classes($layout, $get_array = FALSE) {
 ...
 * @return [type]
 */
function _moshe_load_custom_section($section) {
	$context = array(
		'layout' => array(
...
add_filter( 'timber/twig', function( \Twig_Environment $twig ) {
    ...
    $twig->addFilter( new Twig_SimpleFilter('render_menu', '_moshe_render_menu_recursively') );
    $twig->addFilter( new Twig_SimpleFilter('classes', '_moshe_layout_classes') );
    $twig->addFunction( new Twig_SimpleFunction('get_latest_blog_posts', '_moshe_get_latest_blog_posts') );
    $twig->addFunction( new Twig_SimpleFunction('section', '_moshe_load_custom_section') );
    return $twig;
} );

function moshe_duplicate_post_as_draft() {
	global $wpdb;
	if (! ( isset( $_GET['post']) || isset( $_POST['post'])  || ( isset($_REQUEST['action']) && 'moshe_duplicate_post_as_draft' == $_REQUEST['action'] ) ) ) {
...
add_action( 'admin_action_moshe_duplicate_post_as_draft', 'moshe_duplicate_post_as_draft' );

/*
 * Add the duplicate link to action list for post_row_actions
 */
function moshe_duplicate_post_link( $actions, $post ) {
	if (current_user_can('edit_posts')) {
		$actions['duplicate'] = '<a href="' . wp_nonce_url('admin.php?action=moshe_duplicate_post_as_draft&post=' . $post->ID, basename(__FILE__), 'duplicate_nonce' ) . '" title="Duplicate this item" rel="permalink">Duplicate</a>';
	}
	return $actions;
}

add_filter( 'page_row_actions', 'moshe_duplicate_post_link', 10, 2 );
